...
所有的实体对象定义，均放置在`Game`数据模块中，包括数据通知的结构体和其它原生对象类型。  
因此`Game`数据模块可看作是一个数据容器，禁止在该模块中添加任何业务逻辑；

框架规定所有的业务逻辑，都必须依附于某个实体对象类型，由该实体对象类型中所涉及到的数据，驱动业务流程的推进。  
所有业务系统，均放置在`GameHotfix`逻辑模块中，包括在业务系统中需要用到的管理器或其它封装接口。  
在`GameHotfix`逻辑模块中不允许定义任何数据，所有类均为静态类，内部的函数也全部为静态函数或指定实体对象类型的扩展函数，
由于每个业务系统都依附于某个实体对象类型，所以业务系统的类名一般使用“实体对象类名称+业务功能+System”的格式，
在这个系统类的内部，所有的函数接口或扩展的目标对象，也都是对应的实体对象类型。  

在项目中可能会存在多个模块，比如战斗玩法独立出来的`Battle`数据模块，同时也就一定有个与之配套的逻辑模块，
这个模块的命名格式为：`<数据模块>Hotfix`，以战斗模块为例，那么对应的逻辑模块为：`BattleHotfix`。  
