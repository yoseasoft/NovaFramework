...
### 1.0.27
新增`ConsoleComponent`控制台组件，由引擎内部负责装配。  
控制台提供远程`telnet`指令控制功能，可通过远程指令对当前运行程序进行数据访问、更新，以及模块控制等操作。  

### 1.0.26
将`DebuggerComponent`访问权限调整为`internal`，由引擎内部负责装配，不提供外部进行控制。  

### 1.0.25
将`ApplicationContext`中的模块导入调整到程序集模组中统一管理，减少项目需要配置的地方。  
原模块导入流程修改为设置流程，用于模块装载后的参数设置。  

### 1.0.19
将资源访问接口统一调整为`AssetLoader`管理，从而自动通过模组配置，引擎可以支持使用不同的资源管理框架来进行资源访问。  
将模组热加载流程从`ApplicationContext`解析加载中剥离出来单独执行，确保解析上下文内容时已支持外部模组接口，  
例如使用`AssetLoader`进行上下文配置文件的加载，或将解析后的数据向外部模组进行注入等。  

### 1.0.18
支持实体对象通过给成员变量配置数据标签，从而实现在成员变量的值发生变更时，自动触发对应标签的数据通知。  
修复在系统文件中同时定义扩展通知和全局通知时，全局解析流程中会因为扩展函数的参数不一致问题报错。  
重新规范实体对象配置特性的命名方式。  

### 1.0.17
对当前引擎所支持的四种数据通知类型重新进行业务规划，分为两大体系：
- 行为通知：主要指外部人类的指令行为或业务逻辑行为发生变化时的通知，目前包括输入类和事件类；
- 数据通知：主要指网络下发或实体对象的属性变更时的通知，目前包括消息类和同步类；
根据新的分类标准，来制定业务的开发规范。  

### 1.0.16
重新设计输入响应流程，新增虚拟按键映射，将引擎按键编码转换为一套框架自定义的虚拟按键编码，
同时优化输入捕获接口函数，将参数调整为`VirtualKeyCode`和`InputOperationType`的枚举类型，
方便在函数中使用，无需再进行类型转换，简化工作流程。
...
同时可通过绑定通知回调函数，针对指定数据或某个对象类型的所有数据进行监听，
当其发生变化时，会触发该通知回调函数，处理相应的定制业务；  

### 1.0.12
新增`Installer`模块，支持通过安装器进行功能模组的安装流程；  
将引擎层的部门宏定义调整为`Configure`中的可配置参数，允许用户在开发时动态修改，
目前升级为可配置参数的宏主要包括日志分组控制和调试分析模块相关的宏参数；  

### 1.0.11
修复实体对象及其组件的输入、事件及消息的分发调度在子类尚未激活的状态下，
若基类封装了相关的分发接口，则子类实例化时，会自动注册绑定基类的分发接口，并激活其功能应用在子类的实例上；  
优化扩展函数，为其中部分小规模，高频调用的工具函数，提供`MethodImpl(MethodImplOptions.AggressiveInlining)`特性支持；  
优化符号解析模块，忽略类的泛型函数定义；  
优化统计模块的数据结构，增加关于组件和资产数据的统计；  
