...
框架内的通知管理分为两大体系：**行为通知**和**数据通知**，细分为四类：

| 通知类型 | 分类 | 说明 | 接收标签 |
|---------|-----|------|---------|
| 输入通知 | 行为通知 | 设备 IO（键盘、鼠标、摇杆、触屏等）接收到响应后通知 | `[OnInput]` |
| 事件通知 | 行为通知 | 程序内部逻辑行为触发主动发出的通知 | `[OnEvent]` |
| 消息通知 | 数据通知 | 接收到服务端网络下行数据通知 | `[OnMessage]` |
| 同步通知 | 数据通知 | 实体对象字段/属性变更通知 | `[OnReplicate]` |

> 所有的业务都是由通知传播的数据来推动后续的行为。
> 不同模组的业务系统之间，通过上述通知进行联动。
