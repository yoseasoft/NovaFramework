...
| 判断项 | 结论 | 依据 |
|-------|------|------|
| 邮件是玩家角色的一个业务分支 | 创建 `GComponentWrapper` | 实体的独立行为拆分为组件（规则 2.5） |
| 需要展示邮件列表 UI | 创建 `GViewWrapper` | 每个 UI 资源对应一种视图类型 |
| 邮件数据来自服务端 | 用 `[OnMessage]` 接收 | 消息数据通知 |
| UI 需要知道邮件数据变化 | 通过事件通知 UI 刷新 | 组件通信规则（规则 2.4） |
| 领取附件后加经验 | 通过事件通知属性组件 | 禁止跨组件直接调用 |
| 组件继承 | 继承 `GComponentWrapper` | 与其他实体对象一致，均继承 U 类 |
...
namespace Game
{
    public sealed class AttributeComponent : GComponentWrapper
    {
        public int level;
        public int exp;
        public int health;
...
namespace Game
{
    /// <summary>
    /// 邮件面板视图对象
    /// OnViewConfigure 配置名称必须与 UI 资源名一致
    /// </summary>
    [OnViewConfigure("GameMailPanel")]
    public class MailPanel : GViewWrapper
    {
...
namespace Game
{
    ...
    [GAutomaticallyActivatedComponent(typeof(MailComponent))]      // 新增：自动挂载邮件组件
...
namespace Game
{
    public sealed class SoldierAnimationComponent : GComponentWrapper
    {
...
namespace Game
{
    ...
    public class XXXPanel : GViewWrapper { }
}
...
{
    [GAutomaticallyActivatedComponent(typeof(SomeComponent))]
    public sealed class XXX : Actor { }  // 或 Soldier、GActorWrapper
}
...
// ✅ 正确（继承 G 类或 G 类的子类）
public sealed class Player : Soldier { ... }
public sealed class AttackComponent : GComponentWrapper { ... }
```
...
- [ ] 在 `Game/Component/<功能名>/` 下创建组件，继承 `GComponentWrapper`
- [ ] 定义数据字段和事件结构体（`public struct`）
- [ ] 在 `GameHotfix/Component/<功能名>/` 下创建 System 逻辑类
- [ ] 用 `[OnAwake]` / `[OnDestroy]` 处理初始化和清理
- [ ] 在需要的实体上添加 `[GAutomaticallyActivatedComponent]`

### 6.2 新增一个 UI 面板

- [ ] 在 `Game/View/` 下创建视图类，继承 `GViewWrapper`，`[OnViewConfigure]` 名称与资源名一致
...
- [ ] 在 `Game/Scene/` 下创建场景类，继承 `GSceneWrapper`，添加 `[OnSceneConfigure]`
- [ ] 通过 `[GAutomaticallyActivatedComponent]` 挂载组件
